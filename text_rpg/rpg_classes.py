...
class Warrior(Character):
    def __init__(self, name):
        super().__init__(name, health=150, attack=50, heal=5, mana =0) #각 수치 조정 필요

# 마법사 클래스 
class Mage(Character):
    def __init__(self, name):
        super().__init__(name, health=80, attack=35, heal=20, mana=100) #각 수치 조정 필요

# 성직자 클래스   
class Priest(Character):
    def __init__(self, name):
        super().__init__(name, health=80, attack=25, heal=50, mana=30) #각 수치 조정 필요
...
class Boss(Character):
    def __init__(self, name):
        super().__init__(name, health=300, attack=30, heal=0, mana=0) #각 수치 조정 필요